...
export default function SettingsScreen({ showBackButton = false }: { showBackButton?: boolean }) {
  ...
  useEffect(() => {
    const fetchProStatus = async () => {
      try {
        ...
        console.warn('Failed to sync customer info:', err);
      }
    };
    fetchProStatus();
  }, []);
  ...
  const openStoreReview = async () => {
    ...
    const iosAppId = '<PLACEHOLDER>';

    try {
      if (Platform.OS === 'android') {
        await Linking.openURL(`market://details?id=${androidPackage}`);
      } else {
        await Linking.openURL(`itms-apps://itunes.apple.com/app/id${iosAppId}?action=write-review`);
      }
    } catch {
      if (Platform.OS === 'android') {
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <View style={styles.container}>
        ...
        <Text style={[styles.heading, !showBackButton && styles.headingWithoutBack]}>Settings</Text>
        ...
        <ScrollView>
          <Box>
            ...
            {!isPro && (
              <>
                <Pressable style={styles.menuItem} onPress={handleRestore} disabled={isPro}>
                  <HStack style={styles.textContainer}>
...
const styles = StyleSheet.create({
  ...
  headingWithoutBack: { marginLeft: 0, marginTop: 40 },
  ...
  dropdownTriggerText: { fontSize: 17, fontFamily: 'body-400', color: theme.colors.text1 },
